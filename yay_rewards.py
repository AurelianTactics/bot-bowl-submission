...
class TDReward:
    ...
    ball_progression_reward = 0 #0.005
    ...
    def __call__(self, game: Game):
        ...
        for outcome in game.state.reports[self.last_report_idx:]:
            ...
            if outcome.player is not None:
                ...
                team = outcome.team
            if team == own_team and outcome.outcome_type in TDReward.rewards_own:
                r += TDReward.rewards_own[outcome.outcome_type]
            if team == opp_team and outcome.outcome_type in TDReward.rewards_opp:
                r += TDReward.rewards_opp[outcome.outcome_type]
        self.last_report_idx = len(game.state.reports)
